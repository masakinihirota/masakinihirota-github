---
description: 実装（TDD：RED-GREEN-REFACTORサイクル）
agent: agent
tools:
  [
    "changes",
...
これらの指示書に従い、UIコンポーネントの実装を行ってください。作成した際には、認証後のメニュー /homeのめにゅーにLinkを貼れるページは貼ってください。
記載された要件に基づき、TDD サイクルを通じてページのコードを実装することを目的としています。

ページコンポーネントのコードの実装は

 `.github\instructions\.copilot-instructions.md` 指示書に従います。
