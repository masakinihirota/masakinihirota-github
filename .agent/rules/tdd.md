...
- DB 統合は `RUN_DB_TESTS`（環境変数）で切替える。ローカル実行・CI 実行方針は別途合意。

## 8. DB 統合テストの運用
- `RUN_DB_TESTS=1` が設定されたときのみ統合テストを有効化すること。
- CI での実行は運用者の判断により（コストと時間を考慮）限定実行 or スケジュール実行。
