    ...
    return (
      <div className="max-w-4xl mx-auto p-6">
        ...
        <div className="mt-6">
          ...
          <div className="mt-3 text-sm text-zinc-500 dark:text-zinc-400">人気の作品をここに表示する予定です（未実装）。</div>
        </div>
      </div>
    )
  }
    ...
    if (scope !== 'works' && scope !== 'all') {
      return (
        <div className="max-w-4xl mx-auto p-6">
          ...
          <p className="mt-4 text-sm text-zinc-600 dark:text-zinc-400">現在このスコープはサポートされていません。まずは Works を検索してください。</p>
        </div>
      )
    }
    ...
    if (!rows || rows.length === 0) {
      return (
        <div className="max-w-4xl mx-auto p-6">
          ...
          <div className="mt-6 text-sm text-zinc-600 dark:text-zinc-400">結果は見つかりませんでした。</div>
          <div className="mt-4 text-sm text-zinc-600 dark:text-zinc-400">候補: スコープを <Link href={`/search?q=${encodeURIComponent(q)}&scope=all`} className="text-blue-600 hover:underline">All</Link> にして検索するか、キーワードを変えてみてください。</div>
        </div>
      )
    }
