...
export function SkillEditForm({ skill }: SkillEditFormProps) {
  ...
  const [officialUrl, setOfficialUrl] = useState(skill.officialUrl || "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 実際にはここでAPIを呼び出してスキルを更新
      // await updateSkill(skill.id, { name, category, description, officialUrl });
      console.log("Updating skill:", { id: skill.id, name, category, description, officialUrl });
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-4 pt-4">
              ...
              <Button type="button" variant="outline" asChild>
                <Link href={`/skills/${skill.id}`}>キャンセル</Link>
              </Button>
            </div>
          </form>
        </CardContent>
