...
import { ChevronLeft, Save, Play, AlertTriangle, CheckCircle, Menu, X, ArrowDown, ArrowUp } from 'lucide-react';
...
const GENRE_OPTIONS = [
    'フィクション', 'ファンタジー', 'SF', 'ノンフィクション', 'ホラー', 'ミステリー',
];

const SKILL_OPTIONS = [
    'プログラミング', 'デザイン', 'ライティング', 'イラスト', '音楽',
];
...
const GENERATION_OPTIONS = [
    'すべて', '1960年代', '1970年代', '1980年代', '1990年代', '2000年代',
];
 ...
 */
const ValueBar = React.memo(({ percentage }) => {
    // 5%刻みの表示をシミュレート
...
const Notification = ({ message, type, onClose }) => {
    ...
    return (
        <div className={`fixed top-4 left-1/2 -translate-x-1/2 max-w-lg w-11/12 p-4 rounded-lg shadow-2xl z-50 transition-all duration-300 ${bgColor} text-white flex items-center space-x-3`}>
...
const MatchingSettings = () => {
    ...
    const [settings, setSettings] = useState(() => {
        // 初期状態をダミーデータから設定
        const initialValueImportance = VALUE_OPTIONS.reduce((acc, val) => {
            acc[val.key] = val.key === 'equality' ? 80 : val.key === 'safety' ? 20 : val.key === 'adventure' ? 70 : 50;
            return acc;
        }, {});
    ...
    // 価値観スライダーの更新ハンドラ
    const handleValueChange = useCallback((valueKey, value) => {
        setSettings(prev => ({
            ...prev,
            valueImportance: {
    ...
    const handleCheckboxChange = useCallback((key, item, isChecked) => {
        setSettings(prev => {
    ...
    const handleSave = useCallback(async () => {
        ...
        if (settings.genreFilters.length === 0 || settings.skillFilters.length === 0) {
            setNotification({ message: '⚠️ 少なくとも一つのジャンルとスキルを選択してください。', type: 'error' });
            setIsSaving(false);
            return;
        }
        ...
        try {
            ...
            setNotification({ message: '⚠️ 設定の保存に失敗しました。もう一度お試しください。', type: 'error' });
        } finally {
            setIsSaving(false);
        }
    ...
    const handleStartMatching = useCallback(async () => {
        ...
        try {
            ...
            setNotification({ message: '⚠️ マッチング処理に失敗しました。もう一度お試しください。', type: 'error' });
        } finally {
            setIsStartingMatching(false);
        }
    }, [settings, closeNotification]);
    ...
    const ValueImportanceSection = () => (
        <div className="space-y-6">
            ...
            {VALUE_OPTIONS.map(({ key, label }) => {
                ...
                return (
                    <div key={key} className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-6 bg-gray-50 dark:bg-gray-800 p-4 rounded-lg shadow-sm">
                        <label htmlFor={`slider-${key}`} className="w-full sm:w-32 font-medium text-gray-700 dark:text-gray-300">
                            価値観（{label}）:
                        </label>
                        <div className="flex items-center flex-grow space-x-4">
                            ...
                            <div className="w-32 text-right flex items-center space-x-2">
                                ...
                                <span className="font-semibold text-primary-600 dark:text-primary-400 min-w-[40px] text-right">{percentage}%</span>
                            </div>
                        </div>
                    </div>
                );
    ...
    const CheckboxGroup = ({ title, options, stateKey, currentSelections }) => (
    ...
    const OtherSettingsSection = () => (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 dark:bg-gray-800 p-6 rounded-xl shadow-inner">
                ...
                {/* 世代ドロップダウン */}
                <div className="flex flex-col space-y-2">
                    <label htmlFor="generation-select" className="font-medium text-gray-700 dark:text-gray-300">
                        世代
                    </label>
                    <div className="relative">
                        <select
    ...
    return (
        <div className="min-h-screen bg-gray-100 dark:bg-gray-900 antialiased font-sans">
            ...
            {isSidebarOpen && (
                ...
                >
                    <div className="fixed top-0 left-0 h-full w-64 bg-white dark:bg-gray-900 shadow-xl p-4 transform transition-transform duration-300" onClick={(e) => e.stopPropagation()}>
                        <div className="flex justify-end">
                            <button onClick={() => setIsSidebarOpen(false)} className="p-2 text-gray-600 dark:text-gray-300 hover:text-primary-600">
                                <X className="w-6 h-6" />
                            </button>
                        </div>
                        <Sidebar />
