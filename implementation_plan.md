...
- DB 統合テストは `tests/integration/` にまとめ、ローカル Supabase を前提として実行（CI に含めない）。

---

## タスク分解のテンプレート（各機能で繰り返す）🧭

1. 要件整理 / acceptance criteria を確定（短い箇条書き）
2. 主要画面/コンポーネントの設計 (小さく分割)
3. 失敗するユニットテスト/コンポーネントテストを書く
4. 最小限の実装を追加してテストを通す
5. E2E レベル（必要なら）または統合テストをローカルで作成
6. ドキュメント更新（README / チェックリスト）
