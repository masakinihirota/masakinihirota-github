...
| MVPタスクリスト | `masakinihirota/mvp_tasks.md` |
...
| タスクの同期 | `mvp_tasks.md` の完了状況が設計リポジトリタスクリストに反映されているか |
...
| チェック項目 | 確認方法 |
|-------------|---------|
| タスク完了の実装確認 | `mvp_tasks.md` で ✅ のタスクが実際にコードとして存在するか |
| テストの存在確認 | TDD 方針に従い、完了タスクにテストファイルがあるか |
| DBスキーマの反映 | 要件定義書のエンティティが `drizzle/schema` に存在するか |
| ルーティング構造 | 実装計画のURL設計が `src/app/` 構造と一致するか |
